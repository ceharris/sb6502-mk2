...
id_message:
		ansi_home
		ansi_erase_display
		.byte "SB 6502 Mk2 Monitor", LF, LF
		.byte BEL
		.byte NUL
ihex_prompt: 	
...
@show_hex:
		lda (w0),y
		; If w0 addresses a I/O device, fetching more than once may have
		; undesirable side effects (e.g. popping the stack on the 
		; AM9511). So we save the value to use it again. 
		sta b0
		jsr phex8
@next_hex:
		jsr hex_next
...
@put_next_asc:
		...
		; output this byte
@show_asc:
		; Recover the byte we're processing (saved to avoid)
